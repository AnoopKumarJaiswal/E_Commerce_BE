...
router.get("/products/:id", isLoggedIn, isBuyer , async(req, res) =>{
    try {
           const {id} = req.params
...
router.delete("/products/delete/:id", isLoggedIn, isSeller, async(req,res) =>{
   try {
       const {id} = req.params
       const foundProduct = await Product.findById(id)
       if(!foundProduct)
...
router.patch("/products/edit/:id",isLoggedIn, isSeller, async(req , res) => {
  try {
         const {id} = req.params
         const {name , price, desc, quantity, image , category} = req.body
         if(!id)
